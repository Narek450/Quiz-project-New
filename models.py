...
class QuizAttempt(models.Model):
    ...
    def __str__(self):
        if self.category:
            return f"{self.user.username} - {self.quiz.title} - Category: {self.category.name} - Score: {self.score}"
        else:
            return f"{self.user.username} - {self.quiz.title} - Total Score: {self.score}"

    def total_score(self):
        if self.category:
            return self.score
        else:
            return sum(score.score for score in QuizAttempt.objects.filter(quiz=self.quiz))
...
class ScoresByCategoryManager(models.Manager):
    def all_total_score(self):
        return sum(score.total_score for score in self.all())


class ScoresByCategory(models.Model):
    objects = ScoresByCategoryManager()

    def total_score(self):
        return sum(score.total_score for score in self)
